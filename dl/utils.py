...
class Visdom:
    ...
    def train_loss(self, it, loss):
        ...
        if self.trainLossInit:
            self.vis.line(
                Y=np.array([loss]),
                X=np.array([it]),
                win="{0}_trainloss".format(self.net_type),
                opts={
                    "title": "{0} (train loss)".format(self.net_type),
                    "markers": "true",
                },
            )
    ...
    def test_loss(self, it, loss):
        if self.testLossInit:
            ...
            self.vis.line(
                Y=np.array([loss]),
                X=np.array([it]),
                win=self.net_type + "_testloss",
                update="append",
            )

    def plot_loss(self, type, it, loss):
        if type == "train loss":
            self.train_loss(it, loss)
        if type == "test loss":
            self.test_loss(it, loss)
    ...
    def show_image(self, image, rows: int, cols: int, mask=None):
        ...
        self.vis.image(
            ...
            win="Test Images",
            opts={
                "title": "Test Samples: {0}/{1} - {2}%".format(
                    ntrue, batch_size, ntrue / batch_size * 100
                )
            },
        )
